...
Latest downloader checkpoint: [qBittorrent integration](docs/QBITTORRENT-INTEGRATION.md) includes encrypted administrator settings, read-only connection tests, path mappings and preview, plus submission acknowledgements, transfer/file observations and strict attempt association. Settings and adapter behavior have fixture/browser coverage. [Durable download attempts](docs/DOWNLOAD-ATTEMPTS.md) now add persisted submission, recovery, identity claims and Activity controls. Dispatch is disabled by default pending the remaining integration gates; single-file inspection, repair and full source-to-library certification remain unfinished.
