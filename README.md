...
Latest capacity checkpoint: [Transfer and storage limits](docs/CAPACITY.md) adds installation-wide downloader slots, rolling automatic-transfer budgets and filesystem reservations to the shared downloader/importer. Waiting work resumes through existing schedulers; verified staging recovery avoids reserving a second copy. Settings are administrator-only. Standing list policies and full unattended acquisition remain pending.

Latest automatic-selection checkpoint: [Automatic release preparation](docs/AUTOMATIC-SELECTION.md) ranks fetched source results, inspects up to five torrent manifests, reranks using actual media formats and saves the best eligible candidate through the shared selector. Its private receipt survives reload and opens the existing explicit download action. Complete list-to-dispatch automation and collection/recording coverage remain pending.

Latest aggregation checkpoint: [Book Sources](docs/BOOK-SOURCES.md) combines durable MAM/Prowlarr searches on a title page, keeps source failures independent, saves private format/source/seed preferences and carries the chosen release into the shared acquisition path. Complete series/recording eligibility, native ABB and external-list policies remain pending.
