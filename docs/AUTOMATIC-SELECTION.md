...
Still required for full S06/S07: ABB, magnet metadata resolution, complete collection/recording identity, broader multi-track layouts and full scheduling/inheritance qualification. The explicit per-title action starts from a saved wanted target and fetched search page. [List policies](LIST-POLICIES.md) now add baseline/backfill activation, scheduled search, Either-medium fallback and automatic dispatch using this shared handoff. Bounded single-book evidence does not complete the full unattended list/pack release gate.
