...
The current [testing checkpoint and P0/P1 handoff](TESTING-CHECKPOINT.md) defines the stop point for user testing.
...
2. **Recognize your library:** inspect a known ebook, an audiobook and a work with multiple recordings. Use My Library search for a title, author and narrator, then try media/library/state filters, Recently observed sorting and browser Back/reload. Check the overall ownership badge, medium/version details and the selected ABS link. Resolve an intentionally ambiguous match.
3. **Browse and curate:** use the header search to find local and provider titles together. Local results also support known series, valid ISBNs/ASINs and qualified provider IDs such as `hardcover:42`. Accepted provider matches show current scoped library status and open the existing book; an unmatched result does not prove the book is absent from your library. Try a provider outage and confirm local matches remain usable. Inspect Recent library additions and its ebook/audio filters; observation dates should reflect when the app first saw a copy, while catalog-only books stay out of that shelf. Inspect Your followed lists, filter Hardcover/Goodreads subscriptions, check paused/failed observation status and follow a book or list link into curation. Verify the full-list ownership count and the three-book preview against known holdings. Inspect recommendations and series, create a local list, reorder/remove titles, and follow one external list in Browse mode. Check empty/error/stale states as well as successful results.
...
6. **Small automatic list:** after the manual path works, activate future additions for a small dedicated list. Add one title upstream, observe it arrive once, then test a second overlapping list and an already-owned book. Open Activity to inspect the saved media targets before and after acquisition. Use Background activity to filter task type/status, search a message or operation ID, browse older pages and follow a supported book/list link into its current workflow. Confirm reload/Back preserve the history view and failed refreshes offer Retry activity. Test withdrawing one manual reason while a list reason survives, then include withdrawn history. Keep broad historical backfill off during the first exercise.
