...
- Eight actual ABS 2.36.1 native scanner/API cases and one ebook application import workflow passed with synthetic media; see [native certification](ABS-NATIVE-CERTIFICATION.md). The application case includes API dispatch, durable worker, exact-version confirmation and duplicate skip.
...
| S04 | Reviewed manual import, per-child publication/reservations/fencing and ABS confirmation implemented | Generated covers and broader metadata resolution; automatic matching and grouping corrections; cancellation/replanning; broader media/omnibus/companion handling; full recovery and compatibility matrix |
...
| Evidence | Scope |
|---|---|
| 228 backend tests passing | Prior 214 cases plus 14 integrated publication cases: idempotent and shared-library reservations, exact-version satisfaction, post-rename crash recovery, per-child partial success, final permission/identity fencing, backend metadata/file mismatch, periodic confirmation, credential retry and import-history downgrade protection. Existing corpus includes actual EPUB/MP3/M4B inspection, metadata/export, source integrity, publisher collision/recovery, inventory, canonical grouping, correction and request reconciliation |
| Migration upgrade → downgrade to base → upgrade and Alembic drift check passing on isolated test database | Current eleven migrations and queue namespace; populated correction/acquisition/merge/settings/inspection/import-history downgrade guards tested; not an upgrade from a released production application |
| Frontend TypeScript and production build passing | Current implemented routes and generated API types |
| Browser journey passing | Setup/list/ABS/grant/member/metadata/correction, wanted requests and canonical merge flows; naming, EPUB inspection, catalog-version mapping, plan reload, destination probe, import submission, waiting state, fixture detection, retry and confirmed availability after reload. Desktop/mobile screenshots and no horizontal overflow; one expanded Playwright journey passed |
| Native ABS 2.36.1 certification: eight item cases and one application workflow passing | Actual server/API/manual scanner, publisher hardlinks, conventional/nested version leaves and OPF metadata; native Node 24, synthetic media only. Ebook application API/worker confirmation, ownership and duplicate skip passed. Catalog version seeded in the fixture; watcher, generated covers and the audio application workflow remain pending |
...
- AT-15–AT-19/AT-30: naming, inspection, frozen plans, per-child publication, final authority/identity fencing, journal recovery and ABS confirmation subsets verified. Eight native ABS scanner item cases and the ebook application workflow pass; covers, watcher, grouping corrections, broad media and the complete recovery/scanner matrix remain pending.
