...
| S03 | Inventory path implemented and fixture verified | Complete ABS certification beyond the seven native scanner/API cases; change-event adapter; broader identifier/file-based move resolution; explicit repair/ignore/replacement intents |
| S04 | Naming/inspection/plans, destination probes and publisher primitive implemented | Integrated publication state/reservations/fencing and ABS confirmation; production sidecars; automatic matching and grouping corrections; broader media/omnibus/companion handling; full recovery and compatibility matrix |
...
| Native ABS 2.36.1 certification: seven item cases passing | Actual server/API/manual scanner, publisher hardlinks, conventional/nested version leaves and OPF metadata; native Node 24, synthetic media only. Watcher, production exporter and app publication workflow are not covered |
...
- AT-15–AT-19/AT-30: pure naming, source inspection, frozen review plans and worker generation/replay subsets verified; actual publisher hardlink/copy/collision/recovery cases and seven native ABS scanner item cases now pass; integrated publication, production sidecars, watcher and complete scanner matrix remain pending.
