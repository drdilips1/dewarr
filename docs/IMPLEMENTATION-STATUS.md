...
| Evidence | Scope |
|---|---|
| 370 backend tests passing | Prior 341 cases plus eighteen matching-rule cases and eleven matching API/integration cases. New coverage includes ISBN/ASIN normalization, real embedded evidence, conflicts, stale/forged receipts, duplicate identifiers, canonical bindings, limits and owner/member authorization. Existing coverage includes PDF/CBZ inspection, same-edition formats, companions, publication/cancellation recovery, source integrity, inventory, metadata and acquisition foundations |
...
| Browser journey passing | Setup/list/ABS/grant/member/metadata/correction, wanted requests, canonical merge, organization/publication/cancellation, same-edition consent and companion flows; additionally clear-match selection, separate completeness confirmation, frozen matching evidence and reload. Desktop/mobile screenshots and no horizontal overflow; expanded Playwright journey passed in 53.3 seconds |
