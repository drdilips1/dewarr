...
- Eight actual ABS 2.36.1 native scanner/API cases and ebook plus merged two-track audio application workflows passed with synthetic media; see [native certification](ABS-NATIVE-CERTIFICATION.md). Both application cases include API dispatch, durable worker, exact-version confirmation and duplicate skip; the audio case verifies reviewed group merging and playback order.
- Immutable collection-group review with merge/split, disc/track assignment, explicit exclusions, proposal restore, stale-plan rejection and active-import protection. Multi-file audio confirmation requires the reviewed playback order.
...
| S04 | Reviewed manual import, per-child publication/reservations/fencing and ABS confirmation implemented | Broader metadata resolution; automatic matching and broader collection coverage; cancellation/replanning; broader media/omnibus/companion handling; full recovery and compatibility matrix |
...
| Evidence | Scope |
|---|---|
| 291 backend tests passing | Prior 244 cases plus forty cover unit cases and seven cover-export integration cases. Publication cases include: idempotent and shared-library reservations, exact-version satisfaction, post-rename crash recovery, per-child partial success, final permission/identity fencing, backend metadata/file mismatch, periodic confirmation, credential retry and import-history downgrade protection. Existing corpus includes actual EPUB/MP3/M4B inspection, metadata/export, source integrity, publisher collision/recovery, inventory, canonical grouping, correction and request reconciliation |
| Migration upgrade → downgrade to base → upgrade and Alembic drift check passing on isolated test database | Current thirteen migrations and queue namespace; populated correction/acquisition/merge/settings/inspection/import/grouping/cover-history downgrade guards tested; not an upgrade from a released production application |
| Frontend TypeScript and production build passing | Current implemented routes and generated API types |
| Browser journey passing | Setup/list/ABS/grant/member/metadata/correction, wanted requests and canonical merge flows; naming, EPUB inspection, catalog-version mapping, plan reload, persisted grouping exclusions and proposal restore, destination probe, import submission, waiting state, fixture detection, retry and confirmed availability after reload. Desktop/mobile screenshots and no horizontal overflow; one expanded Playwright journey passed |
| Native ABS 2.36.1 certification: eight item cases and two application workflows passing | Actual server/API/manual scanner, publisher hardlinks, conventional/nested version leaves and OPF metadata; native Node 24, synthetic media only. Ebook and merged-audio application API/worker confirmation, ownership and duplicate skip passed, including reviewed two-track playback order. Catalog versions seeded in the fixture; generated-cover selection and later cover-edit preservation pass for both application workflows; watcher and broader ordering/format cases remain pending |
...
- AT-15–AT-19/AT-30: naming, inspection, frozen plans, per-child publication, final authority/identity fencing, journal recovery and ABS confirmation subsets verified. Eight native ABS scanner item cases and ebook/merged-audio application workflows pass; grouping revision and playback-order checks now supplement this evidence. Initial-cover selection, bounded decoding, recovery and external-artwork preservation now pass their unit/integration/native subsets. Watcher, broad media and the complete recovery/scanner matrix remain pending.
