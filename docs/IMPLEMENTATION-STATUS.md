...
| S04 | Reviewed manual import, per-child publication/reservations/fencing and ABS confirmation implemented | Generated covers and broader metadata resolution; automatic matching and broader collection coverage; cancellation/replanning; broader media/omnibus/companion handling; full recovery and compatibility matrix |
...
| Evidence | Scope |
|---|---|
| 244 backend tests passing | Prior 228 cases plus twelve grouping integration cases and four playback-order rejection cases. Publication cases include: idempotent and shared-library reservations, exact-version satisfaction, post-rename crash recovery, per-child partial success, final permission/identity fencing, backend metadata/file mismatch, periodic confirmation, credential retry and import-history downgrade protection. Existing corpus includes actual EPUB/MP3/M4B inspection, metadata/export, source integrity, publisher collision/recovery, inventory, canonical grouping, correction and request reconciliation |
| Migration upgrade → downgrade to base → upgrade and Alembic drift check passing on isolated test database | Current twelve migrations and queue namespace; populated correction/acquisition/merge/settings/inspection/import/grouping-history downgrade guards tested; not an upgrade from a released production application |
