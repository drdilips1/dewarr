...
- qBittorrent transport now supports current and legacy 5.x acknowledgements, bounded transfer/file observations, full v1/v2 identity and strict attempt/destination association. Seventy-five synthetic HTTP/contract cases cover lost responses, cancellation and unrelated transfers; no production dispatch route is enabled. See [qBittorrent integration](QBITTORRENT-INTEGRATION.md).
...
| S05 | Native MAM browsing/session foundation and qBittorrent transport/association contract implemented and fixture verified | Live MAM/qBittorrent certification; source-to-catalog mapping; downloader connection settings, artifact resolution, persisted dispatch/monitoring and import integration; complete manual acquisition/recovery gate |
...
| Evidence | Scope |
|---|---|
| 485 backend tests passing | Prior 410 cases plus seventy-five qBittorrent transport/association tests covering current, legacy and asynchronous receipts, lost-response lookup, cancellation, full v2/hybrid identity, unrelated transfers and complete-file requirements. MAM coverage includes twenty-nine adapter and eleven source API/integration cases. New coverage includes rich/unknown results, session rotation, proxy fail-closed behavior, credential redaction, concurrency, cooldowns, cancellation, configuration/actor changes and guarded source migration. Prior catalog matching, media inspection, import recovery, source integrity, inventory, metadata and acquisition-foundation coverage remains passing |
| Migration upgrade → downgrade to base → upgrade and Alembic drift check passing on isolated test database | Current fifteen migrations and queue namespace; populated correction/acquisition/merge/settings/inspection/import/grouping/cover/cancellation-history and source-session downgrade guards tested; not an upgrade from a released production application |
| Frontend TypeScript and production build passing | Current implemented routes and generated API types |
| Browser journey passing | Setup/list/ABS/grant/member/metadata/correction, wanted requests, canonical merge, organization/publication/cancellation, same-edition formats, companions and catalog matching; additionally MAM connection, search/detail, secret-free reload and valid empty results. Desktop/mobile screenshots and no horizontal overflow; expanded Playwright journey passes in approximately one minute |
...
- AT-13/AT-30 downloader subset: qBittorrent HTTP fixtures verify submission acknowledgements, no automatic add retry, lost-response observation and strict identity/tag/path association. Persisted dispatch/restart recovery and live-client certification remain pending; this is not the end-to-end transfer gate.
- AT-08: native MAM search/detail, settings, required-proxy failure, safe errors, cookie rotation and cross-process lease/cooldown subsets pass fixtures/browser tests. Account-specific live checks and download artifact behavior are still unverified; no complete MAM/acquisition gate is claimed.
