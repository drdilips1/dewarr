...
- Stalled-job recovery includes diagnostics, fenced ABS inventory, fenced secondary metadata enrichment and idempotent acquisition evaluation/reconciliation. Read-only completed-download inspection uses generation-fenced worker attempts and joins stalled-job recovery. Wanted requests are reevaluated every five minutes while workers are healthy; this does not submit downloads. Every future downloader/import workflow must supply external-state reconciliation before joining recovery.
