...
- Stalled-job recovery includes diagnostics, fenced ABS inventory, fenced secondary metadata enrichment and idempotent acquisition evaluation/reconciliation. Completed-download inspection and destination probes use generation-fenced worker attempts and join stalled-job recovery. Wanted requests are reevaluated every five minutes while workers are healthy; this does not submit downloads. Every future downloader/import workflow must supply external-state reconciliation before joining recovery.
- No API publishes or edits library media. Destination probes create/remove their own temporary hardlinks, copy sentinels and empty folders. The low-level publisher is exercised only by synthetic tests/certification until its durable workflow is complete. Catalog entries never imply ownership.
...
Organization settings now provide a naming preview and saved defaults. The completed-download review screen runs actual read-only inspection and freezes selected group/version mappings; it does not publish files. See [Organization planning](ORGANIZATION-PLANNING.md) for root mapping, supported inspection formats, limits and remaining importer gates. Native audio inspection additionally requires ffmpeg/ffprobe; tests generate tiny original fixtures. [Native ABS certification](ABS-NATIVE-CERTIFICATION.md) documents the real pinned-server test and its boundaries.
