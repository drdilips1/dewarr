...
The restricted worker registers exactly `recovery.scan`, `recovery.reconcile`, `recovery.inventory`, `recovery.publication`, `recovery.lists`, `recovery.outbound` and `recovery.commands`. It has no ordinary or periodic task and no built-in history cleanup. Use `python -m app.jobs.worker --recovery` only with a restored checkpoint. Normal installations use the ordinary worker.
