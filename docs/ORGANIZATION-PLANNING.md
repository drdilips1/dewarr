...
For a fresh installation, open **Library destinations** directly, save the destination, select a tested qBittorrent connection and choose **Test destination route**. No existing download or catalog/import plan is required. The worker resolves the current qBittorrent save folder through its saved path mapping, creates a small exclusive temporary file there, runs the filesystem checks above, removes that file, and verifies the ABS mapping. The save folder must already exist and permit worker writes. This proves the route for the worker-created test file; downloaded files can have different permissions or nested mounts and are always checked during import. A read-only download mount can instead use the existing reviewed-file probe.

Setup probes retain the downloader generation and exact save-folder mapping. Edits, disabled/failed connections, changed roots or a changed destination invalidate the result; a change while the worker is checking discards the result. Repeated commands reuse their operation, and ordinary failures clean up only objects whose identities still match the test. A process kill can leave a uniquely named `.book-search-route-*.tmp` sentinel in the save folder; it is never treated as media or automatically deleted on an unrelated run. The setup test does not add a torrent, create a book, enable automatic importing or enable dispatch.
...
Choose **Review file groups** on a completed inspection. Assign files to an existing group to join them, choose **New group** to separate files, or exclude a file with a reason. Review disc/track numbers for multi-file audio. Every inspected file must be assigned once or explicitly excluded; source bytes and the original inspection are never changed. Unsupported or failed files cannot be promoted into valid media by this editor. Complete ebook and audio media remain separate and alternate audio encodings cannot be combined. Different ebook formats can share one group after explicit same-edition confirmation; each container stays intact. A PDF can be designated as a companion in an audiobook group without becoming an owned ebook. A single omnibus is not split into invented standalone books.
