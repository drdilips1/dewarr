...
## Individual indexer priorities · September 19, 2026

The shared Source preference editor displays MAM, AudiobookBay, a Prowlarr fallback and any explicit indexer priorities. Expand **Individual Prowlarr priorities**, choose **Load Prowlarr indexers**, select an eligible indexer and add it. Each named entry includes its Prowlarr ID so identical names remain distinguishable. A new indexer starts immediately before the fallback; move it up or down to set the desired preference. The existing resolver gives an exact indexer entry its own position and applies the fallback to unlisted Prowlarr origins. Sources omitted entirely have lowest source preference; source exclusions and connection settings determine which sources are queried.

Loading is explicit and uses the existing authenticated indexer-capability API. Merely opening the editor or a form makes no Prowlarr request. Cached names are labeled with their last-loaded time. Excluded/native-MAM overlap, disabled indexers, unavailable search, unsupported download protocols and incompatible book categories show a reason and cannot be added. Unknown categories follow the existing search capability rule. These choices express priority, not proof that a later result will be eligible or available.

Missing, disabled or unreachable indexers do not silently disappear from saved priorities. Existing entries can be reordered or removed during an outage, and a refresh failure disables new additions until a successful lookup. Removal leaves at least one priority; missing standard sources can be added back. The editor enforces the existing 100-entry cap. Changes remain unsaved form values until the enclosing profile/default/request/list action is submitted. Reset Source preference restores that field's inheritance. No indexer credentials, exclusions or connections are modified by these controls, and the API/storage contract is unchanged.
