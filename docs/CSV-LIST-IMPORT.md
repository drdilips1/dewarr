...
Fixtures are synthetic. Goodreads' [export page](https://www.goodreads.com/review/import) requires an account and was not available for live export certification. The parser follows [Python's CSV interface](https://docs.python.org/3.13/library/csv.html); no upstream app implementation was copied. [Hardcover inbound subscriptions](HARDCOVER-LISTS.md) are now implemented separately. Complete provider identity reconciliation, larger-list support and automatic list acquisition remain unfinished PRD requirements.
