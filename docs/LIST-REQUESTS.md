...
Saved [list acquisition policies](LIST-POLICIES.md) now initialize this form's desired media, destinations and effective download limits. Explicit request overrides remain available. Automatic acquisition uses the same request engine with a separate policy-generation reason; it does not reuse this manual preview as standing consent.
