...
Ranking compares source capability/blocked constraints and title/author agreement before the configurable criteria. Normalized title plus agreeing author is corroborating source evidence, never proof of actual file identity. A conflicting author remains unmatched regardless of seed count. Known zero seeds and unknown seeds remain distinct. The shared preference editor supports MAM, AudiobookBay, the Prowlarr fallback and individual Prowlarr indexers. Expand Individual Prowlarr priorities to explicitly load eligible named indexers; saved entries remain editable when a lookup fails. See [download preferences](DOWNLOAD-PREFERENCES.md) for inheritance and fallback behavior.
