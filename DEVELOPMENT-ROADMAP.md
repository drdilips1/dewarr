...
For the current implementation, start with the [refreshed delivery order](IMPLEMENTATION-PLAN.md#15-refreshed-implementation-starting-point-and-delivery-order). Existing fulfillment, review, inbound lists, manual request batches, bounded automatic release preparation and shared capacity accounting have partial evidence. Continue with shared automatic dispatch and list policy activation; complete source/pack coverage and certification before accepting automated-list beta. The earlier eight slices retain dependency rationale, not a fresh instruction to rebuild completed components.
