...
For the existing workspace, use verified [status](docs/IMPLEMENTATION-STATUS.md) and the refreshed delivery order. Retain the working foundation, metadata, inventory, importer, acquisition and list-observation modules. Bounded single-book list-to-library orchestration now has separate implementation evidence. Membership re-addition and canonical merge/undo now have focused lifecycle coverage. Native ABB, full series/identity-correction and inheritance coverage, and complete deployment/live-service qualification remain open. Bounded single-book automatic preparation now has separate implementation evidence; it does not accept the full S06 or S07 stage.
