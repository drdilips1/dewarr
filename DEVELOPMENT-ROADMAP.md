...
September 19, 2026 · v1.9 product baseline · Planning handoff reviewed against committed revision `6777581`, including discovery, curation, list comparison/write-back and the paused-restore foundation. Implementation is partial; no complete stage gate is accepted. Uncommitted work is not acceptance evidence.
...
Current recovery handoff: [historical command retirement](docs/RECOVERY-COMMANDS.md), [durable restored-queue protection](docs/RECOVERY-QUEUE.md) and [saved-approval protection](docs/RECOVERY-APPROVALS.md) add bounded S09-02 safeguards on current schema 0044. Remaining development must supply current-authority activation, persistent automation-policy review, unresolved-effect handling and controlled resume, followed by populated restore qualification. Retain the full product scope above; these safeguards do not constitute a completed release gate.
...
For this workspace, first compare the current revision and uncommitted work with [verified implementation status](docs/IMPLEMENTATION-STATUS.md). Reuse the existing catalog, inventory, selection, list, shared-transfer and import services. The next engineering packet is external-state recovery observation, followed by reviewed reconciliation and controlled resume. Preserve the implemented Hardcover write-back/comparison subsets and complete usability review alongside integration qualification. Qualify actual services, series/version behavior and list automation alongside that work, then close production gates. Section 9 groups the remaining obligations; section 10 gives the current packet order.
...
The current committed starting point is `6777581`. Preserve discovery, community subscriptions, series continuation, local curation, pagination and the delivered [Hardcover write-back](docs/LIST-WRITEBACK.md), [list comparison](docs/LIST-COMPARISONS.md) and [paused restore](docs/RECOVERY.md) subsets. The [recovery observation increment](docs/RECOVERY-OBSERVATIONS.md) supplies isolated reads and saved reports; its exact verification belongs in implementation status, and it does not accept S09. The next sequence is observation → reviewed reconciliation → controlled resume → restore qualification; use the [recovery delivery packets](IMPLEMENTATION-PLAN.md#19-recovery-delivery-packets-and-final-release-order). Reference-load, full usability and all remaining integration gates stay open.
...
| Deliberate Hardcover writes checkpoint / S08-03; qualification pending | Preserve the implemented future-only enablement, durable outbound intent, lost-response reconciliation and conflict/echo controls. Preserve the paged initial difference review and finish actual account qualification and restored-state reconciliation. Keep Goodreads inbound-only and reading status independent. | Add/remove through the supported outbound operation; lose its response, retry and receive the echoed inbound snapshot. Membership converges without duplicate writes, loops or a download changing reading status. Unsupported accounts remain clear and usable. |
| Recovery completion / S09-02 | Finish isolated read-only observations, explicitly reviewed ledger reconciliation, stale-command invalidation and controlled resume. Preserve the persistent restore pause until its prerequisites pass. | Restore while a transfer and partial pack publication are ahead of the backup; identify their current state, recover only authorized work and resume without duplicate dispatch or historical list replay. A completed observation report alone cannot enable automation. |
