...
The [stage closure plan](#17-stage-closure-and-implementation-packets) defines the remaining execution order. Its original packet breakdown starts at `3e6fc70`; the [current handoff](#18-current-development-handoff) accounts for subsequent committed work through `7d506a5`. Section 18 takes precedence over historical next-step statements. Use it with the complete S00–S10 backlog; neither marks partially implemented stages complete.
...
**Planning refresh against `7d506a5`:** the committed baseline includes the reviewed-series controller from `3d26083`, inherited downloader/destination defaults from `3475bdf`, reusable main-book reviews from `0cb4649`, inherited list-driven series acquisition from `e1a462a` and automatic Prefer-pack child fulfillment from `7d506a5`. The [list-derived series increment](docs/LIST-SERIES.md) supplies bounded SC-02/SC-03/SC-05/SC-06 behavior and evidence; exact results are in implementation status. The [automatic Prefer-pack increment](docs/PACK-EXPANSION.md) supplies bounded SC-04 behavior; complete manual-surface and packet/stage qualification remain unfinished. These are implemented subsets with recorded evidence, not completed S06/S07 gates. The subsequent [manual pack preparation increment](docs/MANUAL-PACKS.md) connects finite reviewed children on a saved manual artifact to explicit shared-download submission. Its domain, file and browser evidence is recorded separately; broader source-surface, recording/edition and omnibus qualification remain open. This refresh changes execution guidance only; the v1.9 product requirements, 61 parent packages and acceptance obligations remain intact. No application code or runtime setting changes are required by this planning update.
...
| Area | Starting evidence | Next required outcome |
|---|---|---|
| Committed implementation | `e1a462a`; exact test and deployment scopes are in Implementation Status | Reuse the existing domain, transfer, import and list services; close their remaining acceptance assertions. |
| Prefer-pack expansion | The [automatic expansion increment](docs/PACK-EXPANSION.md) derives covered reviewed children, preserves origin authority and reuses the selected artifact; verification is recorded in Implementation Status | Finish remaining manual-surface, recording/edition/omnibus and actual-service qualification. Do not infer full SC-04 or S06 acceptance from this bounded automatic path. |
| Full product | No complete stage gate is accepted | Execute the remaining sequence below through S09, then the separately scoped S10 releases. |

The initial extra-child fulfillment failure was caused by treating a fulfilled root selection as withdrawn authority. Its correction is verified separately from the broader series-stage requirements. The current contract and evidence distinguish accepted scope, actual per-book fulfillment, fixture verification and actual-service qualification; a completed transfer alone establishes none of those later outcomes.
...
The [route-default increment](docs/ROUTE-DEFAULTS.md) now supplies inherited downloader and per-medium destinations to the manual, list and series surfaces, while freezing actual approved routes at acceptance. This supersedes earlier statements that route defaults are wholly unimplemented; verification and remaining compatibility limits belong in implementation status. Those inherited routes and the subsequent list-derived series increment are the starting point for Prefer-pack incidental expansion and remaining policy qualification.

The reviewed finite-set controller is the execution foundation for series work. The committed increment resolves Just book / Prefer packs / Complete series through the shared settings precedence and carries list-derived reasons/routes into that controller for a bounded reviewed set. Build on the automatic Prefer-pack increment to finish manual-surface integration and broader media/version qualification using the same effective scope and authority contracts. Automatic list expansion must not require a new routine approval for each unambiguous supported series; ambiguous scope enters review without blocking unrelated books. Reuse the current controller and acquisition services rather than creating another transfer path. Prove the whole list-to-series-to-ABS journey before closing S06-05f.
...
The [reusable main-book review increment](docs/SERIES-SCOPE-REVIEW.md) implements SC-01's explicit user-evidence foundation and connects it to finite reviewed requests. It preserves monotonic review revisions, checks current selected evidence, separates withdrawal from accepted request cancellation and never infers main membership from positions. Exact verification belongs in implementation status. The [list-derived series increment](docs/LIST-SERIES.md) now carries inherited policy and originating membership authority into the finite controller, with bounded lifecycle/publication evidence. The [automatic Prefer-pack increment](docs/PACK-EXPANSION.md) now supplies bounded incidental child expansion; finish manual-surface integration and the remaining SC-04–SC-07 media/version, lifecycle and qualification assertions; the presence of a review alone never authorizes acquisition, and these increments do not complete S06/S07.
...
Use the roadmap's [development start and release handoff](DEVELOPMENT-ROADMAP.md#10-development-start-and-release-handoff) for the next acceptance demonstration and complete release artifacts. The planning package is complete enough to execute without another product-preference decision. The application remains under development; the full implementation objective remains open until all required stage outcomes are accepted.
