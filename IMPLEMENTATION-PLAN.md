...
The September 19 planning review uses implementation checkpoint `ec85b46`, schema 0044. S09-02 currently includes [restored-queue protection](docs/RECOVERY-QUEUE.md), [saved-approval protection](docs/RECOVERY-APPROVALS.md), [saved automation review](docs/RECOVERY-AUTOMATION.md), [account permission review](docs/RECOVERY-ACCESS.md), and [ABS/qBittorrent connection repair](docs/RECOVERY-CONNECTIONS.md). Preserve these delivered subsets. The subsequent [source settings and durable verification increment](docs/RECOVERY-SOURCES.md) extends configuration recovery. Metadata-account configuration, actual file-route qualification, unresolved effects/reservations, fresh wanted-work activation and controlled resume remain required. Measured evidence belongs in implementation status; these increments do not close S09 or replace any unmet S00–S10 gate.
...
For the execution starting point, use the roadmap's [development start and release handoff](DEVELOPMENT-ROADMAP.md#10-development-start-and-release-handoff). It consolidates the full stage sequence, selected defaults, integration delivery contracts, release evidence, and remaining recovery, integration, automation and usability work. Section 18 retains source/collection qualification obligations; historical descriptions of a first packet must not restart delivered subsets. Stable package IDs and stage exit gates below remain authoritative.
...
## S09-02 backup and paused-restore foundation · September 19, 2026

The [recovery runbook](docs/RECOVERY.md) documents the implemented version 1 state bundle, offline maintenance exclusion, new-database restore, encryption-key verification, session invalidation and persistent operator-only review. The migration is `0041_restore_checkpoints`. Normal installations keep their current workflows; a restored database cannot start workers or expose ordinary authenticated routes until a later supported reconciliation/resume implementation is available.

This is partial S09-02 progress, not AT-25 acceptance. Continue with a durable external-state census, detection of effects absent from the restored ledger, fresh ABS inventory, per-child publication reconciliation, current external-list observations and an explicit resume decision that cannot replay old commands or launch a historic backlog. Then qualify the populated restore/upgrade journey, measured RPO/RTO and supported container/platform versions. Preserve all earlier unmet S00–S10 requirements and the remaining curation, live-connector, usability and performance gates.


## 19. Recovery delivery packets and final release order

Planning checkpoint: September 19, 2026, committed baseline `6777581`. This section makes the remaining execution order concrete without changing the 61-package backlog or accepting an incomplete stage. The [recovery observation increment](docs/RECOVERY-OBSERVATIONS.md) adds restricted external reads and saved findings on schema 0042. Preserve its bounded verification, recorded in implementation status, while completing the dependent reconciliation and resume packets. An observation report does not close AT-25. Previously delivered write-back, comparison and backup foundations should not be implemented again.

### S09-02 recovery completion

**Scope:** FR-36, FR-22, FR-25, FR-30, FR-34–FR-35; NFR-01–02, NFR-05, NFR-08–09, NFR-11; AT-13, AT-19–25, AT-27, AT-29–30 as applicable. Backend/jobs owns state transitions; integrations owns observations; frontend owns operator review; platform owns the actual restore rehearsal. Assign a person and reviewer when each packet starts.

| Packet | Deliverable and dependency | Acceptance evidence |
|---|---|---|
| Observe external state | After paused restore, a separately restricted worker reads current downloader transfers/files, accessible ABS inventory, publication journals and external-list membership. Persist dated, paginated findings and completeness/error states. It must not register ordinary acquisition, list-write or history-cleanup tasks. | A transfer created after the backup is found without resubmission; a library deletion remains distinguishable from access loss; partial publication and remote list changes remain visible. Zero download, publication, membership-write or destructive cleanup effects occur. Authentication needed for reads is allowed. |
| Review and reconcile | After observation is qualified, prepare a versioned reconciliation plan. Bind each proposed action to current checkpoint, identities, evidence, policy and permissions. Apply selected actions through existing domain services and preserve an audit trail. A match needs corroborating identity, not merely a familiar torrent name. | Adopt only a verified association; preserve unrelated transfers; reconcile each published pack child independently; ambiguous/untracked effects remain held. Replayed commands, concurrent reviewers and changed evidence cannot apply an action twice or silently change its scope. |
| Rebaseline lists and pending commands | Following authoritative observations, reconcile saved membership episodes, pending outbound operations, reservations and stale approvals. RSS may supply additions but never proof of omitted membership deletion. Freeze uncertain outbound effects until independently reconciled. | A new remote member is displayed for catch-up without triggering an unapproved historic backlog. Existing external writes are detected rather than replayed. Overlapping list/manual reasons and current access are retained correctly. |
| Controlled resume | Permit a designated operator to approve a fresh, bounded resume plan only after required reconciliation succeeds. Invalidate old sessions/approvals and fence historical jobs. Activate only current authorized work; unresolved affected workflows remain held. Resume must fail closed if evidence or configuration changes. | Interrupt immediately before and after resume authorization and worker activation. Repeated commands converge, old queue entries cannot bypass the fence, and no second transfer/import/list write occurs. Fresh library confirmation governs availability. |
| Full restore qualification | Rehearse backup, offline restore, observation, resolution and resume on the supported release deployment and real filesystem/client/backend matrix. Preserve the media separately from application-state backup. Measure RPO/RTO against PRD targets. | Transfer ahead of backup, partially published pack, deleted media, missing key, corrupt bundle, provider outage and migration failure each have a documented supported outcome. Record exact versions, schema, images, mounts and evidence. Native PostgreSQL 16 evidence does not certify the proposed PostgreSQL 18 Compose deployment. |

Each packet includes API/worker/UI where relevant, migrations and rollback guidance, authorization, restart behavior and an operator-facing explanation. Observation success means that a report was collected; it does not establish safe reconciliation or authorize resume. Do not implement an unconditional unpause button or document manual deletion of recovery guards as the supported path.

### Remaining v1 release order

1. **Finish and verify the recovery packets above.** Keep affected restored workflows paused until their explicit release conditions pass. Existing normal-installation behavior must retain its regression evidence.
2. **Close source, identity, collection and automation assertions under S03–S07.** Qualify native MAM, ABB, Prowlarr overlap, qBittorrent and ABS against declared supported versions. Demonstrate the common partly-owned trilogy, two narrations, ambiguous child and overlapping-list story. Actual service access can be scheduled alongside recovery work; dependency gates still govern activation.
3. **Finish S08 and the complete ordinary UX.** Preserve the implemented curation and write-back subsets; qualify account capabilities and the remaining available-list rule. Review all eight launch journeys with default settings, keyboard navigation and mobile layouts. No routine per-title approval is allowed on a supported unambiguous Automatic path.
4. **Run reference-load, deployment and release verification.** Close outstanding FR/NFR assertions, migrations, image builds, generated contracts, accessibility and security findings. Measure the documented budgets; do not infer them from unit-test speed or a small fixture run.
5. **Publish a versioned release candidate and accept S09.** Include supported versions/layouts, retained notices, installation/upgrade/restore instructions, operator and user guides, known optional limitations and the requirement-to-evidence ledger. Mandatory P0/P1 failures block production acceptance.
6. **Take S10 extensions separately.** Additional backends such as BookOrbit, clients, recommendation providers, opt-in upgrades/reorganization and SSO retain their six original packages. No unfinished v1 requirement moves here to make a release appear complete.

### Definition of the development handoff

The PRD defines 36 v1 functional requirements, six expansion requirements and 12 nonfunctional requirements. The backlog has 55 v1 packages and six expansion packages; the acceptance plan has 30 core scenarios and five expansion scenarios. Each package receives an accountable owner, reviewer, estimate range and integration-access dependency when scheduled. Reforecast after demonstrated increments; do not equate package count with weeks.

A completed planning package is not a completed application. Update implementation status only from observed evidence, retaining fixture coverage, actual-service qualification and accepted stage gates as separate facts. The scope, stage dependencies and acceptance contracts are sufficient to continue development without reopening settled product preferences.
