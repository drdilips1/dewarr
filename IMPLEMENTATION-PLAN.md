...
Committed revision `93a0dde` includes [saved automation review](docs/RECOVERY-AUTOMATION.md), which adds selective policy pause and rejects historical route qualification on the same schema. Account permission review extends that baseline; its bounded qualification is recorded separately in implementation status. Current configuration, unresolved effects, reservations and wanted-book activation still precede controlled resume and complete AT-25 qualification.
