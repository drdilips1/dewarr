...
The [stage closure plan](#17-stage-closure-and-implementation-packets) defines the remaining execution order. Its original packet breakdown starts at `3e6fc70`; the [current handoff](#18-current-development-handoff) accounts for subsequent committed work through `3475bdf`. Section 18 takes precedence over historical next-step statements. Use it with the complete S00–S10 backlog; neither marks partially implemented stages complete.
...
**P0 design resolution within SC-01:** the recorded provider contract does not establish every member as a main-series book. Do not fill that gap with title heuristics or numeric positions. If reusable user-reviewed scope is needed, specify its separate approval meaning, owner, finite member set, revision, invalidation and withdrawal rules. A one-off reviewed request must not silently become standing permission for future list expansion. Determine the persistence shape in the ticket's ADR; this plan does not mandate a new table versus an existing durable operation record. Unsupported scope enters focused review while unrelated list entries continue.
...
The [reusable main-book review increment](docs/SERIES-SCOPE-REVIEW.md) implements SC-01's explicit user-evidence foundation and connects it to finite reviewed requests. It preserves monotonic review revisions, checks current selected evidence, separates withdrawal from accepted request cancellation and never infers main membership from positions. Exact verification belongs in implementation status. Continue SC-02/SC-03 with inherited policy and list-derived authority, then SC-04–SC-07; the presence of a review does not implement automatic expansion or complete S06/S07.
