...
S06-05a/b now have bounded parser, database, API, browser and migration evidence recorded in implementation status. S06-05c now also has a bounded reviewed target-set implementation: finite saved scope, explicit main-book confirmation, publication warnings, stale-preview protection, independent series reasons and durable receipts through the existing acquisition service. See [Reviewed series requests](docs/SERIES-REQUESTS.md). Shared inherited series policy remains pending within S06-05c; S06-05d/e add compatible pack selection and verified child/omnibus fulfillment. Live provider qualification remains outstanding. These increments do not close S06-05c–f.
