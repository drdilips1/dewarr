...
The current working tree contains [saved automation review](docs/RECOVERY-AUTOMATION.md), which adds selective policy pause and rejects historical route qualification on the same schema. Treat it as in progress until its evidence and revision are recorded. Current permissions/configuration, unresolved effects, reservations and wanted-book activation still precede controlled resume and complete AT-25 qualification.
