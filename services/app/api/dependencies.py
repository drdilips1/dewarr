...
async def current_user(request: Request, db: Database) -> User:
    ...
    if await restore_pending(db):
        ...
        review_action = bool(
            re.fullmatch(
                r"/api/recovery/(?:reconciliations|inventory-reconciliations|publication-reconciliations|list-reconciliations|outbound-reconciliations|command-reconciliations)/[0-9a-f-]{36}",
                request.url.path,
            )
            and request.method == "GET"
            or re.fullmatch(
                r"/api/recovery/(?:reconciliations|inventory-reconciliations|publication-reconciliations|list-reconciliations|outbound-reconciliations|command-reconciliations)/[0-9a-f-]{36}/accept",
                request.url.path,
            )
