...
from app.domain.permissions import preset_bits, sync_user_permissions
...
async def _sync_role(db, user: User, provider: Provider, claims: dict[str, Any]) -> None:
    ...
    if user.role == "admin" and role != "admin":
        # Same lock as account edits, so a group demotion cannot remove the last administrator.
    ...
    if user.role == role:
        return
    # Null permissions follow the role. Explicit grants have to move with it, or a
    # demoted administrator would keep the stored administrator bits.
    if user.permissions is None:
        user.role = role
        if role == "viewer":
            user.can_automate = False
        return
    sync_user_permissions(user, preset_bits(role))
