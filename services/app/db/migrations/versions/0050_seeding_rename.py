...
revision = "0050_seeding_rename"
down_revision = "0049_download_sources"
...
def downgrade():
    if op.get_bind().scalar(sa.text("SELECT EXISTS (SELECT 1 FROM import_destinations WHERE seeding_rename)")):
        raise RuntimeError("Restore a pre-upgrade backup rather than discarding seeding rename settings")
