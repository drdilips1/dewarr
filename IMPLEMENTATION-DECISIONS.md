...
### v1.9 series implementation handoff

Hardcover's [official series guide](https://raw.githubusercontent.com/hardcoverapp/hardcover-docs/main/src/content/docs/api/guides/GettingBooksInSeries.mdx) describes duplicate positions, merged records, partial books and compilations; its website-oriented example chooses the most popular record at each position. For acquisition, our design preserves ambiguity instead: popularity is a discovery signal, not identity evidence. Retain source membership records and explicitly resolve the finite requested set before downloading. A compilation's position is not its complete contents. This is our application contract, not a guarantee from the provider.

The named project repositories and ABS directory/ebook documentation were revisited for this handoff. The Hardcover getting-started URL could not be fetched in this pass; earlier onboarding evidence does not certify current authentication. Shelfmark's current [README](https://github.com/calibrain/shelfmark) documents newer Hardcover token/scoping behavior, so credentials must remain opaque and actual operations must be qualified against the provider's current schema/account permissions. Do not enforce an old JWT shape or copy another application's full scope request when our operation needs fewer permissions.

The v1.9 change is a planning refinement. It records the existing uncommitted series implementation as unverified, adds separate catalog/target/pack acceptance boundaries and retains all prior v1 requirements. It does not establish service compatibility or passing application tests.


### September 19 handoff source check

Revisited the five named project repositories and the official ABS directory and ebook guides for the end-to-end planning handoff. [Seerr](https://github.com/seerr-team/seerr) remains the presentation reference; [MouseSearch](https://github.com/sevenlayercookie/MouseSearch) documents source-preserving organization and path mapping; [Shelfmark](https://github.com/calibrain/shelfmark#project-scope) separates its search/download scope from inventory and background monitoring. These support the selected division of responsibilities, not a claim that their code implements this PRD.

[BookOrbit's license section](https://github.com/bookorbit/bookorbit#license-and-attribution) identifies AGPL-3.0-only with additional terms. Keep its provider-orchestration concepts as a reference under D05; inspect exact file terms before any code reuse. [ABS directory boundaries](https://audiobookshelf.org/docs/documentation/libraries/book-library/directory-structure/) and [ebook handling](https://audiobookshelf.org/docs/documentation/libraries/book-library/ebooks/) remain the starting contracts for actual scanner qualification. This document review does not certify a nested version layout, user's account access, or current deployment compatibility.
