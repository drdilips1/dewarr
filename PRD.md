...
Planning handoff reviewed September 19, 2026 against committed revision `ec85b46`: the roadmap identifies the implementation checkpoint, selected stack, reuse boundaries, integration delivery contracts, stage dependency path and remaining work. The requirement baseline remains unchanged: 36 v1 functional requirements, six expansion requirements and 12 nonfunctional requirements. The 61-package backlog and 35 acceptance scenarios define delivery and verification; implementation checkpoints do not reduce that scope.
...
Narrator controls distinguish **Prefer these narrators** from **Require these narrators**. Preferred names are an ordered ranking preference among eligible audio candidates. Every explicitly required name must be supported by recording evidence; an unknown narrator does not satisfy the requirement. An empty requirement imposes no narrator constraint. Ebook targets ignore inherited audio-only preferences. Requesting an exact catalog recording is a separate, stronger constraint: matching a narrator name alone does not establish recording identity, language, abridgment or release year. Name normalization may handle case and spacing but must not infer unverified aliases. These constraints affect request satisfaction, never the work's existing overall ownership check.

Before selection, shared reasons may combine required narrator names only when their other recording requirements remain compatible; explain that all combined names are required. After selection, attach a stricter reason only when frozen and subsequently verified evidence can satisfy it. Otherwise retain a separate target rather than rewriting the submitted transfer. File inspection and backend confirmation revalidate required recording evidence; tracker claims alone cannot mark the requirement fulfilled.
